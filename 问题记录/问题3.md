...
```rust
fn main() {
    let frame_id = AtkMs901mFrameId::altitude;
    
    match frame_id {
        AtkMs901mFrameId::altitude => println!("Received altitude frame."),
...
use crate::atk_ms901m::AtkMs901maltitudeData;
...
pub async fn get_altitude(
    ...
    if self.get_frame_by_id(
        &mut frame,
        AtkMs901mFrameUploadId::altitude,
        ATK_MS901M_FRAME_ID_TYPE_UPLOAD,
        timeout,
    ).await.is_ok() {
        altitude_dat.roll = ((frame.dat[1] as i16) << 8 | frame.dat[0] as i16) as f32 / 32768.0 * 180.0;
...
157 |         if self.get_frame_by_id(
    |                 --------------- arguments to this method are incorrect
158 |             &mut frame,
159 |             AtkMs901mFrameUploadId::altitude,
...
if self.get_frame_by_id(
    &mut frame,
    AtkMs901mFrameUploadId::altitude as u8,  // 添加 as u8 转换
    ATK_MS901M_FRAME_ID_TYPE_UPLOAD,
    timeout,
).await.is_ok() {
    // ... 后续逻辑不变
...
impl AtkMs901m {
    ...
    pub async fn get_altitude(
        ...
        if self.get_frame_by_id(
            &mut frame,
            AtkMs901mFrameUploadId::altitude.into(),
            ATK_MS901M_FRAME_ID_TYPE_UPLOAD,
            timeout,
        ).await.is_ok() {
            altitude_dat.roll = ((frame.dat[1] as i16) << 8 | frame.dat[0] as i16) as f32 / 32768.0 * 180.0;
...
mod tests {
    ...
    fn test_dma_frame_parsing() {
        ...
        assert_eq!(valid_frames[0].id, AtkMs901mFrameUploadId::altitude as u8);
...
impl FrameStateMachine {
    ...
    pub fn process_byte(&mut self, byte: u8, id: u8, frame_type: u8) -> Option<Result<AtkMs901mFrame, AtkMs901mError>> {
        match self.state {
            ...
            AtkMs901mHandleState::WaitForId => {
                // 如果id是正常的
                if byte >= AtkMs901mFrameUploadId::altitude as u8 && byte <= AtkMs901mFrameUploadId::Port as u8 {
                    self.frame.id = byte;
                    self.frame.check_sum = self.frame.check_sum.wrapping_add(byte);
                    // 进入下一个状态
                    self.state = AtkMs901mHandleState::WaitForLen;
...
impl TryFrom<u8> for AtkMs901mFrameUploadId {
    ...
    fn try_from(value: u8) -> Result<Self, Self::Error> {
        match value {
            0x01 => Ok(AtkMs901mFrameUploadId::altitude),
