...
pub async fn circling_task(
    // 飞行参数
    mut in_flight_args : signals::FlightSub,
){
    defmt::println!("hello from circling_task");
    ...
    loop{
        ...
        // 当前状态不为READY状态才执行降落
        if matches!(flight_mode_now, FlightModeEnum::Ready) && matches!(flight_mode_wanted.mode, FlightModeEnum::Circling) {
            // TODO 调整位姿保持水平
